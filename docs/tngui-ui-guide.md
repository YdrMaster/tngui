...
- 顶部品牌区：第一行为 4px 圆角的品牌图标加“可信网关”，第二行完整显示“Trusted Network Gateway”，不溢出侧栏。
- 导航按钮：
...
- 顶部工具栏：
  ...
  - 注入身份：位于模型选择控件与思考强度控件之间，默认开启的“注入身份” toggle 在中间区域右对齐并紧邻思考强度滑块左侧；标签中的“身份”后缀可点击，点击后在控件下方展开固定 `480px` 宽、`4~12` 行自动增高、超过 12 行后内部滚动的身份提示词浮层；浮层覆盖显示，不压缩聊天记录。默认提示词说明：Agent 由蚂蚁密算公司提供的密态推理方案驱动，模型路由与推理计算完全在 TEE 中执行，运行环境、软件度量与服务身份由远程证明报告验证，用户数据对宿主机、平台运维人员和硬件管理员保持保密，且不得虚构具体硬件型号、证明服务实现细节或其他未提供的体系结构信息。该提示词是密态推理调试页的发送前辅助，不是全局记忆，也不是反向代理行为；开关状态、编辑框展开状态和文本只保留在推理页内存。
...
- 多轮上下文与身份提示词：发送时携带 OpenAI compatible `messages` 数组，包含此前完整完成的 user 消息与 assistant 最终回答 `content`，再追加本轮 user 原文。assistant 的 `reasoning` 绝不进入后续上下文；失败、异常中断或用户主动停止的 assistant 轮次仍显示在 UI 中，但不作为已完成上下文传给模型。“注入身份”开启时，发送层把当前提示词作为独立 `systemPrompt` 参数传给推理命令，后端在最终请求 `messages` 首部生成一条 `role: system` 消息，其后才是既有 user / assistant 上下文与本轮输入；关闭时传 `null`，最终请求与无该调试辅助时保持一致。请求体恒为 `{model, messages, stream: true, reasoning_effort}`，模型身份只来自顶层 `body.model`；前端 / 发送逻辑不注入模型身份头，tngui pre-TNG proxy 按 `body.model` 改写为 `/models/{模型名}/v1/chat/completions`。
- 身份提示词边界：身份提示词只作用于通过“请求调试”页按钮发出的请求；修改开关或文本只影响下一次发送，已进行中的请求不重写、不取消、不重发。提示词文本不做 trim、大小写转换或其他本地格式化，也不作为 user / assistant 气泡渲染或写入聊天历史推导。直接连接本机反向代理的外部客户端请求不经过该调试发送层，请求 body 字节仍按既有规则原样透传，不会被追加 system message 或身份专用头。
...
- 进程内保留：聊天记录、当前选项卡、输入草稿、模型清单/选中模型、思考强度、注入身份开关、身份提示词文本与编辑框展开状态、发送锁、阶段/流式/完成/失败/停止状态、已到达增量和诊断都保留在 GUI 进程内存中。切换到概览或设置页再返回时不重置、不取消请求、不清理阶段 timer；后台流式输出继续推进，返回后 transcript 滚动到底部。进程退出后这些推理页状态全部丢弃，身份设置恢复为默认开启、默认提示词与编辑框收起，不写入设置缓存、tng 配置或任何文件。
...
- 配置锁定：导入 JSON、导出 JSON 按钮左侧提供锁定 toggle，默认开启。锁定时，远程证明开关、参考值服务地址、远端类型、远端字段、本机绑定、端口输入、原始 JSON 文本框和“应用回填表单”按钮均不可交互；关闭锁定后恢复可编辑。锁定 toggle、导入 JSON、导出 JSON、状态卡与远端证明报告导出不受锁定影响；锁定状态不写入 TNG 配置 JSON 或设置缓存。
...
- 远程证明服务配置：当前 ingress 开启远程证明时，TNG 配置内容下方展示标题为“远程证明服务配置”的配置块，提供全局可编辑的“参考值服务地址”；入口关闭远程证明时不展示该配置块，已有地址仍保留在设置缓存中供下次开启时恢复。界面不展示环境变量、设置缓存或 TNG 配置 JSON 的实现说明。
  - 参考值服务地址首次启动或设置缓存缺失/无效时预填 `https://rvs.cloud.misuan.com`；导入、回填或有效设置缓存中的显式值优先，不会被默认值覆盖。即使配置 JSON 缓存损坏，独立缓存中的有效地址也会保留。
  - 参考值服务地址属于 TNG 配置状态，受配置锁定 toggle 控制；修改后与设置页其他 TNG 配置一起在离开设置时自动保存，若 tng 正在运行则自动重启。
  - 启动或重启远程证明版 tng 时，GUI 把当前地址写入子进程环境变量 `RATS_TEE_VERIFIER_URL`，保证进程使用的地址与界面提交值一致。该地址不写入 `tng-runtime.json` 的 TNG 配置 schema。
