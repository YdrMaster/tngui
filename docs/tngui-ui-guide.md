...
- 左侧“请求”卡：
  ...
  - 输入内容：可编辑的 prompt textarea，发送后不清空，可修改后重发；高度按内容自动调整（4–16 行），不能手动拖拽。
  - 发送测试请求按钮内图标与文字垂直居中；按钮下方不保留额外锁形提示条。
  - 发送测试请求：满足可用条件时按 OpenAI 兼容格式，POST 到 tngui 反代对外端点 `http://<绑定地址（默认 127.0.0.1）>:<本机端口>/v1/chat/completions`；模型身份只来自请求体顶层 `body.model`；前端 / 发送逻辑不注入模型身份头，tngui pre-TNG proxy 会按 `body.model` 改写为 `/models/{模型名}/v1/chat/completions`。
- 右侧“响应”卡：
  - 发送过程中显示五个阶段点、现有线性进度和当前阶段的单卡 SecureFlow；阶段发生推进时，上一张阶段卡向上淡出，新阶段卡从下方淡入。
...
不可发送时，请求面板显示引导卡，其中的“前往设置”直接切换到设置视图，不再弹窗要求用户自行点侧边栏。
...
- 远程证明服务配置：任意 ingress 开启远程证明时，TNG 配置内容下方展示标题为“远程证明服务配置”的配置块，提供全局可编辑的 RVS 地址；全部 ingress 关闭远程证明时不展示该配置块，已有 RVS 地址仍保留在设置缓存中供下次开启时恢复。
  - RVS 地址首次启动或设置缓存缺失/无效时预填 `https://rvs.tsk.com:9443`；设置缓存有效时恢复用户填写值，不会被默认值覆盖。
  - RVS 地址属于 TNG 配置状态，修改后与设置页其他 TNG 配置一起在离开设置时自动保存；若 tng 正在运行则自动重启。
